...
//
template <size_t S>
class aligned_stack_memory_buffer
{
public:
  static const size_t SIZE = S;

  aligned_stack_memory_buffer (void) = default;
  char *get_ptr (void)
  {
    return m_buf;
  }
...
template <size_t S>
class resizable_memory_buffer : protected aligned_stack_memory_buffer<S>
{
public:

  resizable_memory_buffer ()
    : aligned_stack_memory_buffer<S> ()
    , m_own_buf (NULL)
    , m_size (S)
  {
    //
    ...
    if (new_size > m_size)
      {
        ...
        std::memcpy (new_buf, m_ptr, m_size);
...
template <size_t S>
class append_buffer : protected resizable_memory_buffer<S>
{
public:
  append_buffer ()
    : resizable_memory_buffer<S> ()
    , m_offset (0)
  {
    //
  }
  ...
  {
    resize (size + m_offset);
    std::memcpy (get_ptr () + m_offset, data, size);
    m_offset += size;
  }
